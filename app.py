...
def init_sheets():
    try:
        ...
        if "riwayat" not in existing: sheet.add_worksheet("riwayat", rows=100, cols=35)
        if "v12_memory" not in existing: sheet.add_worksheet("v12_memory", rows=100, cols=3)
        if "v12_predictions" not in existing: sheet.add_worksheet("v12_predictions", rows=500, cols=8)
        if "riwayat_actual" not in existing: sheet.add_worksheet("riwayat_actual", rows=100, cols=7)
...
def load_v12_memory():
    ...
    try:
        ...
        for row in sheet.get_all_records():
            t = row.get('ticker')
            if t and row.get('data'):
                try: mem[t] = json.loads(row['data'])
                except: pass
    except Exception as e: st.error(f"Gagal memuat V12 memory: {e}")
    return mem
...
def save_v12_memory(mem):
    ...
    except Exception as e: st.error(f"Gagal menyimpan V12 memory: {e}")

def load_v12_predictions(ticker):
    try:
        sheet = get_gsheet().worksheet("v12_predictions")
        for row in sheet.get_all_records():
            if row.get('ticker') == ticker: return row
        return None
    except Exception as e:
...
def save_v12_prediction(ticker, close_price, factor_signals):
    try:
        ...
        for k in FACTOR_KEYS: new_row[f'sig_{k}'] = factor_signals.get(k, 0.0)
        ...
        for i, row in enumerate(records):
            if row.get('ticker') == ticker:
                ...
                break
        if row_index:
            sheet.update(f'A{row_index}:H{row_index}', [[new_row[h] for h in headers]], value_input_option='RAW')
        else:
            if not records: sheet.insert_row(headers, 1)
            sheet.append_row([new_row[h] for h in headers], value_input_option='RAW')
    except Exception as e: st.error(f"Gagal menyimpan prediksi: {e}")
...
def coppock_curve(prices, rP1=14, rP2=11, wP=10):
    ...
    def wma(data,per):
        ...
        w = np.arange(1,per+1)
        return np.dot(data[-per:], w) / w.sum() if len(data) >= per else 0.0
...
def get_adaptive_weights(ticker, regime):
    ...
    for k in FACTOR_KEYS:
        ...
        elif acc<0.35: w = max(w*0.2, WEIGHT_MIN/2)
    ...
    final = {}
    for k in FACTOR_KEYS:
        final[k] = max(WEIGHT_MIN, min(WEIGHT_MAX, 0.6*w_pri[k] + 0.4*w_pri[k]*sm[k]*len(FACTOR_KEYS)))
...
def simpan_riwayat(ringkasan):
    ...
    except Exception as e: st.error(f"❌ Gagal menyimpan riwayat: {e}")

def muat_riwayat_dari_sheets():
    try:
        sheet = get_gsheet().worksheet("riwayat")
        return list(sheet.get_all_records())[:50]
    except Exception as e:
...
def muat_riwayat_actual():
    ...
    except Exception as e: st.error(f"Gagal memuat actual: {e}")
    return data
...
def simpan_riwayat_actual(waktu, saham, actual_data):
    ...
    except Exception as e: st.error(f"Gagal menyimpan actual: {e}")

def integrate_actual_to_v12(waktu, saham, actual_data):
    try:
        ticker = saham
        last_pred = load_v12_predictions(ticker)
        if not last_pred: return
        factor_signals = {}
        for k in FACTOR_KEYS:
            key = f'sig_{k}'
            if key in last_pred: factor_signals[k] = float(last_pred[key])
        outcome = actual_data.get('Outcome', '')
        if outcome == 'Win': actual_return = 1.0
        elif outcome == 'Loss': actual_return = -1.0
        else: actual_return = 0.0
    ...
    except Exception as e: st.error(f"Gagal integrasi V12: {e}")
...
def analisis_saham_dengan_ai(data_saham, riwayat, api_key):
    ...
    if riwayat:
        ...
        for r in riwayat[:10]:
            ...
            if ai_insight: base += f" | AI Insight: {ai_insight[:120]}..."
            riwayat_text += base + "\n"
...
"""
    ...
    except Exception as e: return None, f"Gagal menghasilkan insight AI: {str(e)}"
...
def analisis_riwayat_global(riwayat_data, api_key):
    ...
    try:
        response = model.generate_content(prompt)
        return response.text.strip(), None
    except Exception as e: return None, f"Gagal menghasilkan insight: {str(e)}"
...
if 'v12_memory' not in st.session_state: st.session_state.v12_memory = load_v12_memory()
if "riwayat" not in st.session_state: st.session_state.riwayat = muat_riwayat_dari_sheets()
if "riwayat_actual" not in st.session_state: st.session_state.riwayat_actual = muat_riwayat_actual()
...
with st.sidebar:
    ...
    trading_style = st.radio("⏱️ Gaya Trading:", ["Swing Trade (Mingguan)", "Day Trade (Harian)"], index=0, key="trading_style")
    if "Day Trade" in trading_style: st.caption("Menggunakan data 5-menit (5 hari terakhir)")
    else: st.caption("Menggunakan data harian (2 tahun terakhir)")
    ...
    ticker_input = f"{ticker_raw}.JK" if ticker_raw and not ticker_raw.endswith(".JK") else ticker_raw

    col1, col2 = st.columns(2)
    with col1: run_btn = st.button("🚀 ANALISIS", use_container_width=True)
    with col2:
    ...
    if "riwayat_page" not in st.session_state: st.session_state.riwayat_page = 0
    ...
    if search_query: riwayat_data = [r for r in riwayat_data if search_query.lower() in r.get('Saham', '').lower()]
    ...
    if total_pages > 1:
        ...
        with col1:
            if st.button("◀ Sebelumnya", disabled=(st.session_state.riwayat_page == 0), key="prev_page"):
                st.session_state.riwayat_page = max(0, st.session_state.riwayat_page - 1)
        with col2: st.markdown(f"<div style='text-align:center; color:#8892b0;'>Hal. {st.session_state.riwayat_page + 1} / {total_pages}</div>", unsafe_allow_html=True)
        with col3:
            if st.button("Selanjutnya ▶", disabled=(st.session_state.riwayat_page >= total_pages - 1), key="next_page"):
    ...
    if display_riwayat:
        for r in display_riwayat:
            ...
            conf_val = 0
            try: conf_val = float(r.get('Confidence', '0%').replace('%',''))
            except: pass
            conf_text = "Tinggi ▲" if conf_val >= 70 else ("Sedang ►" if conf_val >= 50 else "Rendah ▼")
            est_ret = 0
            try: est_ret = float(r.get('Est_Return', '0%').replace('%','').replace(',',''))
            except: pass
            ...
            with st.expander(expander_title):
                ...
                with c1: st.markdown(f"""<div style="margin-top: 0px;"><label style="color:rgb(255, 255, 255); font-size:14px; margin:0 0 4px 0; display:block;">{tplabel}</label><div style="color:rgb(0, 255, 204); font-size:24px; font-weight:700; line-height:1.2;">Rp {r.get('TP_Harga','?')}</div></div>""", unsafe_allow_html=True)
                with c2: st.markdown(f"""<div style="margin-top: 0px;"><label style="color:rgb(255, 255, 255); font-size:14px; margin:0 0 4px 0; display:block;">{sllabel}</label><div style="color:rgb(239, 68, 68); font-size:24px; font-weight:700; line-height:1.2;">Rp {r.get('SL_Harga','?')}</div></div>""", unsafe_allow_html=True)
                ...
                if actual_data and (actual_data.get('Actual_High') or actual_data.get('Outcome')):
                    ...
                    if actual_data.get('Outcome'):
                        warna_outcome = {'Win': '🟢', 'Loss': '🔴', 'Not Touched': '⚪'}.get(actual_data['Outcome'], '')
                    ...
                    if st.session_state.get(show_key, False):
                        with st.form(key=f"form_actual_{waktu_key}_{saham_key}"):
                            ...
                            outcome = st.selectbox("Outcome", options=["", "Win", "Loss", "Not Touched"], format_func=lambda x: "Pilih Outcome" if x == "" else x)
                            entry_miss = False
                            if outcome in ["Win", "Loss"]:
                                entry_miss = st.checkbox("Entry Tidak Tersentuh", value=False, help="Centang jika harga tidak menyentuh zona entry.")
                            if st.form_submit_button("Simpan"):
                                ...
                                st.rerun()
                if r.get("AI_Insight", "").strip(): st.caption(f"💡 {r['AI_Insight'][:150]}")
        st.caption(f"📋 Menampilkan {start_idx+1}-{min(end_idx, total_items)} dari {total_items} riwayat" + (f" (hasil pencarian '{search_query}')" if search_query else ""))
    else:
        st.caption("❌ Tidak ada riwayat yang cocok." if search_query else "Belum ada riwayat.")
    ...
    if "gemini_api_key" not in st.session_state: st.session_state.gemini_api_key = get_api_key()
    api_key = st.text_input("Gemini API Key", type="password", value=st.session_state.gemini_api_key, placeholder="AIza...")
    ...
    if st.button("🗑️ Hapus Semua Riwayat"):
        try:
            get_gsheet().worksheet("riwayat").clear()
            st.session_state.riwayat = []
            st.success("Riwayat dihapus!")
        except Exception as e: st.error(f"Gagal menghapus riwayat: {e}")
    ...
    st.subheader(f"📅 Kalender Bursa {now_jkt.strftime('%Y')}")
    libur_bursa = {
    ...
    def dalam_jam_perdagangan(hour, minute):
        return (hour == 9 and minute >= 0) or (10 <= hour < 12) or (hour == 12 and minute == 0) or (hour == 13 and minute >= 30) or (hour == 14) or (hour == 15 and minute == 0)
...
def get_google_news_rss(query_str, num=5):
    ...
    try:
        ...
        return [{'title': e.get('title','').strip(), 'summary': re.sub('<[^<]+?>','',e.get('summary','')), 'source':'Google News'} for e in feed.entries[:num]], None
...
def estimate_theta_ou(close_series):
    ...
    return -coeff[1] if coeff[1]<0 else 0.05
...
if run_btn:
    if not ticker_input: st.warning("⚠️ Kode saham tidak boleh kosong!"); st.stop()
    ...
    with st.spinner("🤖 Mengunduh data dan memproses analitika kuantitatif..."):
        ...
        def get_regime_row(row):
            ...
            if a>adx_threshold:
                if h>e20 and e20>e50:
                    return ("Strong Bullish 🚀","RISK-ON 🔥") if (m>mom_median_th or z>-1.5) else ("Bullish 📈","RISK-ON 🔥")
                elif h<e20 and e20<e50:
                    return ("Panic Sell 🚨","RISK-OFF 🛑") if (m<mom_median_th or z<-1.5) else ("Bearish 🔻","RISK-OFF 🛑")
        ...
        avg_gain = gain.rolling(14).mean().iloc[-1]; avg_loss = loss.rolling(14).mean().iloc[-1]
        ...
        if is_daytrade:
            ...
            if not df_daily.empty:
                ...
                if not df_daily_filtered.empty:
                    ...
                    hi_daily = float(prev_day['High']); lo_daily = float(prev_day['Low']); cl_daily = float(prev_day['Close'])
                else:
                    hi_daily = float(df['High'].iloc[-1]); lo_daily = float(df['Low'].iloc[-1]); cl_daily = float(df['Close'].iloc[-1])
            else:
                hi_daily = float(df['High'].iloc[-1]); lo_daily = float(df['Low'].iloc[-1]); cl_daily = float(df['Close'].iloc[-1])
            if hi_daily != lo_daily:
                pp = (hi_daily + lo_daily + cl_daily) / 3
                r1 = 2 * pp - lo_daily; s1 = 2 * pp - hi_daily
                r2 = pp + (hi_daily - lo_daily); s2 = pp - (hi_daily - lo_daily)
            else: pp = r1 = s1 = r2 = s2 = cl_daily
        else:
            hi = lo = cl = None
            for i in range(1, min(6, len(df))):
                row = df.iloc[-i]
                h_val = float(row['High']); l_val = float(row['Low']); c_val = float(row['Close'])
                if h_val != l_val and h_val > 0 and l_val > 0: hi, lo, cl = h_val, l_val, c_val; break
            if hi is None: hi = float(df['High'].iloc[-1]); lo = float(df['Low'].iloc[-1]); cl = float(df['Close'].iloc[-1])
        ...
        if is_daytrade:
            ...
            res20 = float(df['High'].iloc[-bars_per_day:-1].max()) if len(df) >= bars_per_day else float(df['High'].max())
            breakout_label = f"Breakout Sesi Sebelumnya ({bars_per_day} bar)" if len(df) >= bars_per_day else "Breakout N-Bar"
        else:
            res20 = float(df['High'].iloc[-21:-1].max()) if len(df) >= 21 else float(df['High'].max())
        ...
        if is_daytrade: backtest_window = min(500, len(df))
        else: backtest_window = 126
        ...
        for i in range(len(df_back)):
            ...
            if in_position:
                ...
                if "AVOID" in curr_sig or i==len(df_back)-1: trades.append((curr_close-entry_price)/entry_price); in_position=False
            else:
        ...
        if trades:
            ...
            annual_factor = np.sqrt(bars_per_day_map.get(actual_interval,54)*252) if is_daytrade else np.sqrt(252)
        ...
        if is_daytrade and trades_bt < 15:
            st.warning(f"⚠️ Backtest hanya menghasilkan **{trades_bt}** sinyal trading dalam {backtest_window} bar. Interpretasikan dengan hati‑hati.")
        ...
        estimasi_label = "Estimasi Sesi Berikutnya" if is_daytrade else "Estimasi Besok"
        prob_label = "Prob Naik Sesi Berikutnya" if is_daytrade else "Prob Naik Besok"
        ...
        vol_surge_pct = ((avg_vol_5 / avg_vol_20) - 1) * 100 if avg_vol_20 > 0 else 0.0
        ...
        rsi_status = "Overbought" if rsi14 > 70 else ("Oversold" if rsi14 < 30 else "Normal")
        zscore_val = df['ZScore'].iloc[-1]
        zs_status = "Overbought" if zscore_val > 2 else ("Oversold" if zscore_val < -2 else "Normal")
        vs_status = "Tinggi" if vol_surge_pct > 50 else ("Rendah" if vol_surge_pct < -30 else "Normal")
        coppock_val, coppock_prev = coppock_curve(df['Close'].values)
        coppock_turning_up = coppock_val > coppock_prev and coppock_prev <= 0
        coppock_status = "Turning Up" if coppock_turning_up else ("Rising" if coppock_val > coppock_prev else "Falling")
    ...
    st.success(f"✅ Analisis Berhasil: {ticker_input} | Closing Price: Rp {harga_terakhir:,.0f}")

    col1,col2,col3 = st.columns(3)
    col1.metric("Sinyal Eksekusi", signal)
    col2.metric(estimasi_label, f"Rp {fraksi_bei(est_besok):,.0f}",
                f"50% range: Rp {fraksi_bei(low_est):,.0f} - {fraksi_bei(up_est):,.0f}")
    ...
    with st.expander("🔍 Lihat Detail Analisis"):
        ...
        p3.metric("Pivot", f"Rp {fraksi_bei(pp):,.0f}"); p4.metric("S1", f"Rp {fraksi_bei(s1):,.0f}")
        p5.metric("S2", f"Rp {fraksi_bei(s2):,.0f}")
        ...
        t2.metric(estimasi_label, f"Rp {fraksi_bei(est_besok):,.0f}")
        t3.metric("Entry Zone", f"Rp {fraksi_bei(entry_low):,.0f} - {fraksi_bei(entry_high):,.0f}")
        t4.metric("Take Profit", f"Rp {fraksi_bei(tp_harga):,.0f} (+{tp_pct:.1f}%)", "Target Profit")
        t5.metric("Stop Loss", f"Rp {fraksi_bei(sl_harga):,.0f} (-{sl_pct:.1f}%)", "Stop Loss")
    ...
    with st.expander("🧬 V12 Adaptive Engine (Coppock, Self‑Learning)", expanded=True):
        st.info("⚙️ **Bagian ini adalah otak adaptif dari QuantRisk Pro.** Engine secara otomatis mempelajari akurasi setiap faktor teknikal berdasarkan riwayat analisis kamu.")
        if not is_daytrade:
            st.markdown("### 📈 Coppock Curve & Beta IHSG")
            coppock_insight = "🟢 **Turning Up**" if coppock_turning_up else ("🟢 **Rising**" if coppock_rising else "🔴 **Falling**")
            st.metric("Coppock Curve", f"{coppock_val:.3f}", coppock_insight)
        st.markdown("### ⚖️ Bobot Adaptif per Faktor")
        adaptive_w = get_adaptive_weights(ticker_raw, regime)
        if is_daytrade: display_adaptive_w = {k:v for k,v in adaptive_w.items() if k != "Coppock"}
        else: display_adaptive_w = adaptive_w
        st.bar_chart(pd.DataFrame.from_dict(display_adaptive_w, orient='index', columns=['Weight']))
        ...
        if mem:
            ...
            with col_a: st.caption("✅ Accuracy"); st.bar_chart(pd.Series(acc_data))
            with col_e: st.caption("⚠️ Error EMA"); st.bar_chart(pd.Series(err_data))
        ...
        if last_pred:
            ...
            update_v12_memory(ticker_raw, last_signals, actual_return, returns.std())
            st.success(f"✅ Memory updated! Actual return: {actual_return*100:.2f}%")
        else: st.info("ℹ️ Tidak ada prediksi sebelumnya.")
        factor_signals = {
        ...
        st.caption("📌 Prediksi hari ini telah disimpan.")
    ...
    if st.session_state.get("gemini_api_key"):
        with st.spinner("🧠 AI sedang menganalisis hasil dan riwayat..."):
            ...
            }, [r for r in st.session_state.riwayat if r['Saham']==ticker_input][:10], st.session_state.gemini_api_key)
            if not error_ai and hasil_ai:
                st.markdown(f'<div class="ai-insight-card"><h3>🤖 Insight AI</h3><p>{bersihkan_teks_ai(hasil_ai)}</p></div>', unsafe_allow_html=True)
    ...
    **Fitur Utama:**
    ...
    > **Disclaimer:** Dashboard ini merupakan alat bantu analisis kuantitatif. Keputusan investasi tetap tanggung jawab masing-masing.
    """)

    st.markdown("---")
    st.subheader("📈 Informasi Pasar Terkini (IHSG)")

    periode_pilihan = st.selectbox("Periode data IHSG:", options=["1d", "5d", "1mo"],
                                    format_func=lambda x: {"1d": "1 Hari", "5d": "5 Hari", "1mo": "1 Bulan"}[x], index=0, key="ihsg_period")

    interval_candidates = ["1m", "5m", "15m", "30m", "60m", "1d"] if periode_pilihan == "1d" else (["5m", "15m", "30m", "60m", "1d"] if periode_pilihan == "5d" else ["1d"])
    ...
    if not df_ihsg_preview.empty and len(df_ihsg_preview) >= 2:
        ...
        if periode_pilihan == "1d":
            ihsg_change = (ihsg_close - prev_close) / prev_close * 100 if prev_close else (ihsg_close - float(df_ihsg_preview['Close'].iloc[-2])) / float(df_ihsg_preview['Close'].iloc[-2]) * 100
        else:
            ihsg_change = (ihsg_close - open_period) / open_period * 100 if open_period > 0 else 0.0
        ihsg_high = float(df_ihsg_preview['High'].max())
        ihsg_low = float(df_ihsg_preview['Low'].min())
        if open_price is None or open_price == 0: open_price = open_period
        vol_val = df_ihsg_preview['Volume'].sum() if interval_terpakai in ("1m","5m","15m","30m","60m") else float(df_ihsg_preview['Volume'].iloc[-1])
        ...
        if PLOTLY_AVAILABLE:
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=df_ihsg_preview.index, y=df_ihsg_preview['Close'], mode='lines',
                                     line=dict(color=line_color, width=1.5), fill='tozeroy', fillcolor=area_color,
                                     name='IHSG', hovertemplate='<b>%{x|%d %b %H:%M WIB}</b><br>Close: %{y:,.0f}<extra></extra>'))
            fig.add_hline(y=ihsg_high, line_dash='dot', line_color='rgba(255,255,255,0.4)', line_width=1)
            fig.add_annotation(x=0.5, y=ihsg_high, xref='paper', yref='y', text=f'H {ihsg_high:,.0f}',
                               showarrow=False, font=dict(size=9, color='rgba(255,255,255,0.6)'),
                               bgcolor='rgba(15, 17, 22, 0.7)', bordercolor='rgba(255,255,255,0.3)',
                               borderwidth=1, borderpad=4, xanchor='center', yanchor='bottom')
            fig.add_hline(y=ihsg_low, line_dash='dot', line_color='rgba(255,255,255,0.4)', line_width=1)
            fig.add_annotation(x=0.5, y=ihsg_low, xref='paper', yref='y', text=f'L {ihsg_low:,.0f}',
                               showarrow=False, font=dict(size=9, color='rgba(255,255,255,0.6)'),
                               bgcolor='rgba(15, 17, 22, 0.7)', bordercolor='rgba(255,255,255,0.3)',
                               borderwidth=1, borderpad=4, xanchor='center', yanchor='bottom')
            fig.update_yaxes(range=[float(df_ihsg_preview['Low'].min())*0.998, float(df_ihsg_preview['High'].max())*1.002])
            chart_title = {"1d": "IHSG Hari Ini", "5d": "IHSG 5 Hari Terakhir", "1mo": "IHSG 1 Bulan Terakhir"}.get(periode_pilihan, "IHSG")
            fig.update_layout(title=dict(text=chart_title, x=0.01, xanchor='left', font=dict(size=14, color='#e0e0e0')),
                              template="plotly_dark", height=400, margin=dict(l=10, r=20, t=40, b=10), dragmode='pan',
            ...
            st.plotly_chart(fig, use_container_width=True, config={'modeBarButtonSize': 4, 'displaylogo': False})
        else: st.line_chart(df_ihsg_preview['Close'])
    elif not df_ihsg_preview.empty and len(df_ihsg_preview) == 1:
        st.metric("IHSG", f"{float(df_ihsg_preview['Close'].iloc[-1]):,.0f}")
        st.warning("Data IHSG hanya tersedia 1 titik.")
    else: st.warning("Data IHSG tidak tersedia.")
