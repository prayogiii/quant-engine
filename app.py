...
# ====================== FUNGSI BARU: HITUNG SISA BAR SAMPAI TUTUP SESI ======================
def hitung_bars_remaining(now_jkt, actual_interval, bars_per_day_map):
    """
    Hitung sisa bar (candle) sampai penutupan sesi bursa IDX hari ini.
    now_jkt : datetime dengan tz Asia/Jakarta
    actual_interval : str, salah satu '5m','15m','30m','60m'
    bars_per_day_map : dict, mapping interval ke jumlah bar normal per hari
    """
...
with st.sidebar:
    ...
    if display_riwayat:
        for idx, r in enumerate(display_riwayat):
            # ... (tampilan riwayat sama seperti sebelumnya, tidak diubah)
                    ...
                    if st.session_state.get(show_key, False):
                        with st.form(key=form_key):
                            ...
                            if entry_miss:
                                ...
                                outcome = st.selectbox("Outcome", options=["", "Win", "Loss", "Not Touched"],
...
if run_btn:
    ...
    with st.spinner("🤖 Mengunduh data dan memproses analitika kuantitatif..."):
        ...
        atr14_val = df['TR'].rolling(14).mean().iloc[-1]      # ATR dalam rupiah (MENTAH, tidak di-scale)
        atr_pct = (atr14_val / harga_terakhir_asli) * 100       # untuk display saja, tetap mentah
        
        # === PATCH V12: hitung sisa bar dan scale ATR untuk Entry Zone & TP ===
        now_jkt = datetime.now(pytz.timezone("Asia/Jakarta"))   # waktu terkini untuk hitung sisa sesi
        if is_daytrade:
            bars_remaining = hitung_bars_remaining(now_jkt, actual_interval, bars_per_day_map)
            if pd.isna(atr14_val):
                atr_tp_entry_basis = atr14_val
            else:
                atr_tp_entry_basis = atr14_val * (bars_remaining ** 0.5)
        else:
            bars_remaining = None
            atr_tp_entry_basis = atr14_val  # Swing Trade: tanpa scaling
        ...
        # ============ ENTRY ZONE (ADAPTIF) – pakai atr_tp_entry_basis ============
        atr_pct_scaled = (atr_tp_entry_basis / harga_terakhir_asli) * 100 if not pd.isna(atr_tp_entry_basis) else 0.0
        if s1 >= harga_terakhir * 0.98:
            entry_low = s1
        else:
            entry_low = harga_terakhir * (1 - atr_pct_scaled/100)
        if "STRONG BUY" in signal:
            entry_high = harga_terakhir
        else:
            entry_high = harga_terakhir * (1 - 0.3 * atr_pct_scaled/100)
        if entry_low > entry_high:
            entry_low, entry_high = entry_high, entry_low
        min_entry_width = 0.5 * atr_tp_entry_basis
        ...
        if is_daytrade:
            ...
            min_ticks_dist = 2 * fraksi_step(harga_terakhir)
            sl_dist = max(min_ticks_dist, base_sl_dist)
            sl_harga = harga_terakhir - sl_dist
        else:
            # Swing Trade SL menggunakan atr14_val mentah
        ...
        # ============ TAKE PROFIT RANGE – pakai atr_tp_entry_basis ============
        if r1 > harga_terakhir:
            tp_low = r1
        else:
            tp_low = harga_terakhir + tp_mult_low * atr_tp_entry_basis
        if r2 > harga_terakhir:
            tp_high = r2
        else:
            tp_high = harga_terakhir + tp_mult_high * atr_tp_entry_basis
        if tp_low > tp_high:
        ...
        # Breakout (adaptif) – tidak berubah
        if is_daytrade:
        ...
        with col1:
            ...
            ''', unsafe_allow_html=True)
            # --- Caption transparansi Day Trade ---
            if is_daytrade and bars_remaining is not None:
                interval_minutes_map = {"5m": 5, "15m": 15, "30m": 30, "60m": 60}
                menit_tersisa = bars_remaining * interval_minutes_map.get(actual_interval, 5)
                col1.caption(
                    f"📐 Entry zone & TP diproyeksikan ke estimasi **{bars_remaining} bar** "
                    f"(~{menit_tersisa} menit) tersisa sampai penutupan sesi. "
                    "SL tetap fixed 4% dari harga."
                )
        with col2:
        ...
        # --- DETAIL EXPANDER --- (tidak ada perubahan selain caption di atas, tidak perlu diulang)
        with st.expander("🔍 Lihat Detail Analisis (Berita, Fundamental, Backtest, dll)"):
            # ... isi expander sama persis seperti sebelumnya, tidak perlu diubah
        ...
        # V12 Adaptive Engine (sama seperti sebelumnya)
        with st.expander("🧬 V12 Adaptive Engine (Coppock, Self‑Learning)", expanded=True):
            ...
            if not is_daytrade:
                ...
                if beta_ihsg > 1.2:
                    beta_insight = f"⚠️ **Beta Tinggi ({beta_ihsg:.2f})** – Saham lebih volatile dari IHSG."
                elif beta_ihsg > 0.8:
                    beta_insight = f"✅ **Beta Moderat ({beta_ihsg:.2f})** – Pergerakan selaras dengan IHSG."
                else:
                    beta_insight = f"🛡️ **Beta Rendah ({beta_ihsg:.2f})** – Saham defensif."
                st.metric("Beta IHSG", f"{beta_ihsg:.2f}x")
                st.caption(beta_insight)
            ...
            st.caption("Bobot dihitung otomatis berdasarkan **akurasi historis** masing‑masing faktor.")
            adaptive_w = get_adaptive_weights(ticker_raw, regime)

            if is_daytrade:
                display_adaptive_w = {k: v for k, v in adaptive_w.items() if k != "Coppock"}
                st.caption("ℹ️ Faktor **Coppock** tidak ditampilkan dalam bobot adaptif untuk Day Trade.")
            else:
            ...
            if display_adaptive_w:
                ...
                interpretations = {
                    "Momentum": "Sinyal momentum (harga 5 hari) paling berpengaruh – pasar sedang *trend-following*.",
                    "AI_Senti": "Sentimen berita paling berpengaruh – pergerakan saham banyak dipicu oleh berita.",
                    "MeanRev": "*Reversal* ke rata-rata (Z-Score) paling berpengaruh.",
                    "Beta_IHSG": "Beta IHSG paling berpengaruh – saham sangat terpengaruh oleh pergerakan pasar.",
                    "Coppock": "Coppock Curve paling berpengaruh – sinyal jangka panjang mendominasi."
                }
            ...
            if mem:
                ...
                st.info("Belum ada data memori untuk ticker ini.")

            st.markdown("### 🔁 Proses Self‑Learning")
            st.caption("Setiap analisis, engine membandingkan prediksi sebelumnya dengan harga aktual.")
        ...
        if st.session_state.get("gemini_api_key"):
            with st.spinner("🧠 AI sedang menganalisis hasil dan riwayat..."):
                data_ai = {
                    ...
                    "TP%": f"{tp_pct_low:.1f}% - {tp_pct_high:.1f}%", "SL%": f"{sl_pct:.1f}",
    ...
    try:
        ...
        if not df_ihsg_preview.empty and len(df_ihsg_preview) >= 2:
            ...
            if PLOTLY_AVAILABLE:
                ...
                fig.update_layout(title=chart_title, template="plotly_dark", height=400,
                                  margin=dict(l=10, r=20, t=40, b=10), dragmode='pan')
                st.plotly_chart(fig, use_container_width=True)
            else:
